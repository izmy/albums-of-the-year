          ...
          <TableBody>
            {usersVotesCount.map((u) => (
              <StyledTableRow key={u.user}>
                ...
                <StyledTableCell scope="row">
                  {u.votes.find((vote) => vote.type === "global-2024")?.count ??
                    0}
                </StyledTableCell>
                <StyledTableCell scope="row">
                  {u.votes.find((vote) => vote.type === "nomination-czech-2024")
                    ?.count ?? 0}
                </StyledTableCell>
                <StyledTableCell scope="row">
                  {u.votes.find((vote) => vote.type === "czech-2024")?.count ??
                    0}
                </StyledTableCell>
              </StyledTableRow>
            ))}
            <StyledTableRow style={{ background: "#cae8ff" }}>
              ...
              <StyledTableCell>
                <strong>
                  {usersVotesCount.reduce((acc, curr) => {
                    const count =
                      curr.votes?.find(
                        (v) => v.type === "nomination-global-2024"
                      )?.count ?? 0;
                    return acc + count;
                  }, 0)}
                </strong>
              ...
              <StyledTableCell>
                <strong>
                  {usersVotesCount.reduce((acc, curr) => {
                    const count =
                      curr.votes?.find((v) => v.type === "global-2024")
                        ?.count ?? 0;
                    return acc + count;
                  }, 0)}
                </strong>
              ...
              <StyledTableCell>
                <strong>
                  {usersVotesCount.reduce((acc, curr) => {
                    const count =
                      curr.votes?.find(
                        (v) => v.type === "nomination-czech-2024"
                      )?.count ?? 0;
                    return acc + count;
                  }, 0)}
                </strong>
              ...
              <StyledTableCell>
                <strong>
                  {usersVotesCount.reduce((acc, curr) => {
                    const count =
                      curr.votes?.find((v) => v.type === "czech-2024")?.count ??
                      0;
                    return acc + count;
                  }, 0)}
                </strong>
